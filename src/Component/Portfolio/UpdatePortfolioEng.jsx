...
export default function UpdatePortfolioEng({
  editData,
  setEngData,
  setArticleEng,
  image,
  ...
  // set articleForcms data
  useEffect(() => {
    let rows = [];
    editData?.articleForCMS?.map((e) => {
      const allRow = {
  ...
  const formik = useFormik({
    initialValues: {
      title: editData?.title?.replace(/<\/?(?!a)(?!p)(?!img)\w*\b[^>]*>/gi, ""),
      discription: editData?.discription?.replace(
        /<\/?(?!a)(?!p)(?!img)\w*\b[^>]*>/gi,
        ""
      ),
    },
