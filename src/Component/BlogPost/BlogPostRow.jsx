...
//components
import "./blogpostrow.scss";
import DeleteForm from "../DeleteForm/DeleteForm";
import { DELETE_BLOGPOST } from "../../Schema/blogpost";
...
export default function CleaningMaterailsRow({
  ...
  const toUpdateBlog = () => {
    navigate("/blogpost/updateblogpost", {
      state: { row },
    });
  };
  ...
  const [deleteBlog, { data, loading, error }] = useMutation(DELETE_BLOGPOST, {
    onCompleted: ({ deleteBlog }) => {
      if (deleteBlog?.success === true) {
        setOpenSuccess(true);
        setSuccesstMessage(deleteBlog?.message);
        setRefetch();
        handleCloseDelete();
      } else {
        setOpenError(true);
        setErrorMessage(deleteBlog?.message);
      }
    },
