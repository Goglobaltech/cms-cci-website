...
import CleaningMaterailsRow from "../Component/CleaningMaterails/CleaningMaterailsRow";
import AlertMessage from "../Component/AlertMessage/AlertMessage";

export default function CleaningMaterails() {
  let navigate = useNavigate();
  const toCreate = () => {
    navigate("/cleaningmaterails/createcleaningmaterails");
  };
  // Alert Message
  ...
  const { error, data, refetch } = useQuery(GET_PRODUCTS_BY_TYPE_PAGINATION, {
    variables: {
      ...
      keyword: keyword,
      pagination: true,
      typeCategory: "CleaningMaterails",
    },
    onCompleted: ({ getProductByTypePagination }) => {
      setTableData(getProductByTypePagination);
      // console.log("CleaningMaterails::", getProductByTypePagination);
    },
  });
  ...
  return (
    <div className="products-page">
      <Stack direction="row" spacing={2} sx={{ mb: 3 }}>
        ...
        <Stack direction="column" justifyContent="center">
          <span className="page-title">Cleaning materails</span>
        </Stack>
      </Stack>
